...
def run_pace_analysis(df):
    ...
    if df.empty or 'lat' not in df.columns or 'lon' not in df.columns or 'time' not in df.columns:
    ...
    df['distance'] = [0.0] + [geodesic(
        (df.loc[i-1, 'lat'], df.loc[i-1, 'lon']),
        (df.loc[i, 'lat'], df.loc[i, 'lon'])).meters for i in range(1, len(df))]
    df['cum_dist'] = df['distance'].cumsum()
    df['speed_kmh'] = df['distance'] / df['seconds'].diff().replace(0, np.nan).fillna(1) * 3.6
    ...
    st.altair_chart(
        alt.Chart(df).mark_line(color='darkred').encode(
            x=alt.X('seconds', title='Time (s)'),
    ...
    for i in range(len(df) - 1):
        ...
        color = (
            'green' if spd < 8 else
            'orange' if spd < 14 else
            'red')
        folium.PolyLine([(lat1, lon1), (lat2, lon2)], color=color, weight=4, opacity=0.9).add_to(m)
    ...
    st.markdown(f"**Coefficient of Variation:** {cv:.2%} — lower means more consistent pacing")
